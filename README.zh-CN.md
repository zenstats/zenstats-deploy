...
| 变量 | 必填 | 默认 | 说明 |
|------|------|------|------|
| `ZENSTATS_SECRET_KEY` | **生产必填** | — | JWT 签名密钥，留空生产启动失败 |
| `ZENSTATS_DOMAIN` | 否 | `localhost` | 域名（非 localhost 自动 SSL） |
| `DB_PASSWORD` | 建议修改 | `postgres` | 数据库密码 |
| `ZENSTATS_MAXMIND_LICENSE_KEY` | 否 | — | MaxMind GeoIP Key（免费注册，留空自动降级） |
...
- [API 项目](https://github.com/zenstats/zenstats) ↗
- [前端项目](https://github.com/zenstats/zenstats-web) ↗

---

## License

**AGPL-3.0** — 详见 [LICENSE.md](LICENSE.md)。
